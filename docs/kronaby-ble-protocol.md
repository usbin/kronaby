...
#### `dnd` — 방해금지 (펌웨어 업데이트 후 지원 확인, 2026-04-17)
```
파라미터: [활성화(Bool), 시작시, 시작분, 종료시, 종료분]
예: [true, 22, 0, 6, 0]  → 22:00~06:00 방해금지 활성화
    [false, 22, 0, 6, 0] → 22:00~06:00 방해금지 비활성화
주의: 활성화 값은 MsgPack Bool (0xC3=true, 0xC2=false), 정수 1/0이 아님
```
...
| 11 | 버튼 매핑 | 상단 5조합 + 하단 4조합 + 확장입력모드(16종) + 날짜/배터리/걸음수 표시 | ✅ |
...
| 17 | DND 방해금지 | `dnd` 명령 (펌웨어 업데이트 후 지원, Bool 파라미터) | ✅ |
| 18 | 세계시간 | timezone2 | ✅ |
...
| 25 | 확장입력모드 바늘 | 진입→0시(자동), 완료→명령번호 위치, 취소→즉시복귀 | ✅ |
| 26 | 오늘 날짜 확인 | 시침→월(5분 단위), 분침→일 (5초 표시 후 복귀) | ✅ |
| 27 | 배터리 잔량 표시 | 양 바늘→배터리%(0~59분 눈금, 5초 표시) | ✅ |
| 28 | 걸음수 확인 | 양 바늘→걸음수/목표 비율(0~59분 눈금, 5초 표시) | ✅ |
...
#### 6시간 주기 BLE 강제 재연결 — iOS ANCS 보안 세션 만료 대응
iOS가 BLE 페어링 보안 세션을 내부적으로 만료시키면 시계의 ANCS 구독이 끊어질 수 있음.
`cancelPeripheralConnection` 호출 (intentionalDisconnect=false) → 자동 재연결 트리거 → 전체 서비스 재검색 + 설정 재전송.
BLE 연결 자체가 끊기므로 OS 레벨의 보안 세션이 완전히 리셋됨.
**주의: `complications` 세 번째 값 `18`의 정확한 의미가 불명이므로 임의 변경 금지** (0으로 보내면 ANCS 깨짐 확인됨)
...
| 항목 | 상태 | 비고 |
|------|------|------|
| 앱별 알림 필터 (Bundle ID) | ✅ 구현 완료 | 공식앱 BLE 캡처로 확인: `ancs_filter [idx, 0xFFFFFF, 0, bundleIdPrefix, vibSlot]` (2026-04-17) |
| 날씨 | 미구현 | 외부 API 연동 필요 |
...
- 할당 가능 액션: 폰 찾기, 오늘 날짜 확인, 배터리 잔량 표시, 걸음수 확인, 음악 제어, 위치 기록, IFTTT Webhook, iOS 단축어, URL 요청

### 확장입력모드 (16종)
- **진입**: 하단 길게 누름 → 진동 1회 + 바늘 0시(12시, recalibrate 자동)
- **입력**: 하단 1회=0, 2회=1 → 4자리 2진수 입력 (0000~1111 = 0~15)
- **완료**: 4자리 완성 → 진동 2회 + 바늘→명령번호 위치 → 3초 유지 → 복귀 → 명령 실행
...
### 전체 commandMap (74개, Kronaby Nord 펌웨어 — 업데이트 전)
```
...
**참고: 펌웨어 업데이트 후 commandMap이 변경됨 (2026-04-17 확인)**
공식 앱을 통해 펌웨어가 업데이트되면 commandMap에 `dnd` 등 새 명령이 추가되고 ID가 재배치됨.
BLE 캡처에서 확인된 변경: `datetime` 13→19, `ancs_filter` 4→5, `dnd` 신규 추가 (ID 28).
정확한 전체 매핑은 "기기 정보 삭제" 후 재연결하여 새 핸드셰이크로 확인 필요.
